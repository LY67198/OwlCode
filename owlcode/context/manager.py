"""对话上下文管理器：实现大型工具结果落盘、全对话摘要和压缩恢复。"""
...
from owlcode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    estimate_tokens,
)
from owlcode.serialization import build_messages
...
SESSION_SUBDIR = ".owlcode/session/tool-results"
...
def load_replacement_records(session_dir: Path) -> list[ContentReplacementRecord]:
    ...
    Returns:
        解析出的 ContentReplacementRecord 列表。文件不存在时返回空列表。
    """
    path = session_dir / REPLACEMENT_RECORDS_FILENAME
    if not path.exists():
        return []
...
def reconstruct_replacement_state(
    ...
    Args:
        messages: 当前消息列表，用于确定 candidate IDs。
        records: 持久化的替换记录。
        inherited_replacements: 继承的替换映射（优先级低于 records 中的直接记录）。

    Returns:
        重建的 ContentReplacementState。
    """
...
def make_persisted_preview(content: str, file_path: Path) -> str:
    """为已落盘的大型结果生成带预览信息的替换文本。

    Args:
        content: 原始内容（用于估算大小和截取预览）。
        file_path: 落盘文件路径。

    Returns:
        格式化的替换文本，包含文件路径和前 2KB 预览。
    """
...
def _snip_stale_messages(
    ...
    """裁剪历史消息中过旧的大型工具结果，将其替换为截断版本。

    保留最近 KEEP_RECENT_TURNS 轮的工具结果原文，更早的轮次中超过
    OLD_RESULT_SNIP_CHARS 字符的结果会被截断为简短摘要。
...
def should_auto_compact(last_input_tokens: int, context_window: int) -> bool:
    ...
    Args:
        last_input_tokens: 上次输入的 token 数。
        context_window: 模型上下文窗口大小。
...
def extract_summary(llm_output: str) -> str:
    """从 LLM 输出中提取 <summary> 标签内的内容。

    Args:
        llm_output: LLM 原始输出。

    Returns:
        提取出的摘要文本。未找到标签时返回原文本。
    """
...
def build_compact_messages(
    ...
    """构建压缩后的消息列表（单条 user 消息，包含摘要和可选附件）。

    Args:
        summary: 对话摘要文本。
        attachment: 恢复附件（文件快照、技能等）。
        has_keep_tail: 是否有原样保留的近期尾部消息。
        transcript_path: 完整会话记录文件路径。
...
async def auto_compact(
    ...
    """自动压缩对话历史：对旧对话生成摘要，保留近期原文。

    根据当前 token 用量判断是否需要压缩，决定保留尾部消息窗口，
    调用 LLM 生成前缀摘要，替换历史并向 session 层返回 CompactBoundary。

    Args:
        conversation: 当前对话管理器。
        client: LLM 客户端（需支持 stream 接口）。
        context_window: 模型上下文窗口大小。
        session_dir: 会话目录，用于落盘和清理。
        protocol: 消息序列化协议，默认 "anthropic"。
        manual: 是否为手动触发。
        breaker: 可选的熔断器。
        recovery: 可选的恢复状态，用于生成附件。
        tool_schemas: 可选的工具 schema 列表。
        transcript_path: 可选的完整会话记录路径。

    Returns:
        压缩成功返回 CompactEvent（含 boundary），无需压缩返回 None，
        失败返回错误描述字符串。
    """
    ...
    for attempt in range(max_retries):
        try:
            from owlcode.tools.base import StreamEnd, StreamEvent, TextDelta
