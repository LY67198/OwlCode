...
from owlcode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from owlcode.conversation import ThinkingBlock as ConvThinkingBlock
...
from owlcode.hooks import HookContext, HookEngine, ToolRejectedError
from owlcode.hooks.engine import HookNotification
from owlcode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from owlcode.tools import ToolRegistry
from owlcode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class StreamCollector:
    """LLM 流式响应收集器，将 StreamEvent 转换为 AgentEvent。"""
    ...
    async def consume(
        ...
        """消费 LLM 流并产出 AgentEvent。

        将底层的 StreamEvent（TextDelta、ToolCallStart 等）转换为
        上层的 AgentEvent（StreamText、ToolUseEvent 等），同时累积
        完整的 LLMResponse。
...
def partition_tool_calls(
    ...
    连续标记为并发安全的工具归入同一个并行 batch，不安全的工具
    单独成批，保证串行执行。

    Args:
        tool_calls: LLM 返回的工具调用列表。
        registry: 工具注册表，用于查询并发安全性。
...
class StreamingExecutor:
    ...
    async def collect_results(self) -> list[_ToolExecResult]:
        """等待所有已提交的任务完成，按提交顺序返回结果。

        Returns:
            _ToolExecResult 列表，异常会被捕获并转为错误结果。
        """
        if not self._tasks:
            return []
...
class Agent:
    """OwlCode 的核心 Agent，管理 LLM 交互、工具执行和对话循环。

    负责协调 LLM 流式响应、工具调用执行、权限检查、上下文压缩、
    Hook 触发和 memory 提取等核心流程。
    """
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".owlcode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".owlcode" / "plans"
    ...
    def activate_skill(self, name: str, prompt_body: str) -> None:
        """激活一个技能，将其 prompt 注入到系统提示词中。

        Args:
            name: 技能名称。
            prompt_body: 技能的 prompt 正文。
        """
    ...
    def set_agent_catalog(self, catalog: str, catalog_list: list[tuple[str, str]] | None = None) -> None:
        ...
        Args:
            catalog: agent 目录的格式化字符串。
            catalog_list: agent 目录列表，用于协调者模式。
        """
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        注入环境上下文和长期记忆，然后进入 LLM 调用 -> 工具执行 ->
        结果反馈的循环，直到无工具调用或达到最大迭代次数。

        Args:
            conversation: 对话管理器。

        Yields:
            AgentEvent 子类实例，供 UI 层消费。
        """
    ...
    async def _execute_tool(
        ...
        对于需要用户批准的权限请求，会 yield PermissionRequest 并等待
        异步 future 完成。
        ...
        Yields:
            (ToolResult, elapsed, is_unknown) 元组或 PermissionRequest。
        """
        tool = self.registry.get(tc.tool_name)
        start = time.monotonic()
        is_unknown = False
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from owlcode.permissions.rules import Rule, extract_content
    ...
    async def manual_compact(
        ...
        调用 auto_compact 并以 manual=True 模式运行，压缩后重新注入
        环境上下文和长期记忆。
        ...
        Returns:
            成功时返回 CompactNotification，失败时返回 ErrorEvent。
        """
    ...
    async def run_to_completion(
        ...
        """以非流式方式运行 Agent 直到任务完成。

        适用于非交互式场景（如 -p 模式），不 yield 事件，直接返回最终文本。

        Args:
            task: 用户任务描述。
            conversation: 可选的已有对话管理器，为 None 则新建。
            event_callback: 可选的事件回调，用于上报 token 用量和文本。
    ...
    async def _execute_tool_noninteractive(
        ...
        对于需要用户批准的权限请求，在 DONT_ASK 模式下自动批准，
        其他模式下返回错误。
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        ...
        超过 SINGLE_RESULT_CHAR_LIMIT 则持久化到磁盘并返回预览，
        超过 MAX_OUTPUT_CHARS 则截断。
        ...
        Returns:
            处理后的文本（预览或截断后的内容）。
        """
        from owlcode.context.manager import (
