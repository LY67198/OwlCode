"""Worktree 清理：自动检测并删除过期的临时 worktree。"""
...
from owlcode.worktree.changes import has_unpushed_commits, has_worktree_changes
...
async def cleanup_stale_worktrees(manager: WorktreeManager, cutoff_hours: int) -> int:
    """清理超过指定时间的过期 worktree。

    仅删除匹配临时命名模式、无变更、无未推送提交且不在当前会话中的 worktree。

    Args:
        manager: WorktreeManager 实例。
        cutoff_hours: 过期时间阈值（小时）。
...
async def start_stale_cleanup_task(
    ...
    """启动一个持续运行的后台清理任务，定期清理过期 worktree。
