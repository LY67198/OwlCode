...
from owlcode.config import ConfigError, load_config
from owlcode.hooks import HookConfigError, HookEngine, load_hooks
...
def main() -> None:
    ...
    解析命令行参数，加载配置，初始化 hook 引擎，然后根据模式
    启动交互式应用或处理单次非交互式 prompt。
    """
    # 先确保 .owlcode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".owlcode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".owlcode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="owlcode", description="OwlCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from owlcode.app import OwlCodeApp
    from owlcode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    """非交互模式：用给定的 prompt 运行一次 agent 循环并打印结果。

    适用于命令行传入 -p 参数的单次执行场景，不启动 TUI，
    直接输出最终响应文本到 stdout。
    ...
    from owlcode.client import create_client, resolve_context_window
    from owlcode.conversation import ConversationManager
    from owlcode.memory.instructions import load_instructions
    ...
    from owlcode.tools import create_default_registry
    ...
    from owlcode.worktree import WorktreeManager
