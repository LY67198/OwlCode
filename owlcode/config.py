...
class ProviderConfig:
    ...
    def resolve_api_key(self) -> str:
        """解析 API key：优先使用配置中的显式值，否则从环境变量获取。

        Returns:
            解析出的 API key 字符串，未找到则返回空字符串。
        """
        if self.api_key:
            return self.api_key
    ...
    def get_max_output_tokens(self) -> int:
        ...
        Returns:
            配置中指定了正数则返回该值，否则根据 thinking 模式返回默认值
            （thinking 开启时 64000，关闭时 8192）。
        """
...
def resolve_env_vars(value: str) -> str:
    """解析字符串中的环境变量引用（格式：${VAR_NAME}）。

    Args:
        value: 可能包含 ${VAR_NAME} 引用的字符串。

    Returns:
        所有环境变量引用已被替换的字符串。未定义的环境变量保留原样。
    """
...
def build_child_env(declared_env: dict[str, str] | None) -> dict[str, str]:
    ...
    Args:
        declared_env: 声明式环境变量映射，值中可包含 ${VAR} 引用。

    Returns:
        包含 PATH 继承和已解析变量的环境变量字典。
    """
    env: dict[str, str] = {}
...
def _merge_config(base: AppConfig, override: AppConfig) -> AppConfig:
    """将 override 配置合并到 base 配置中（base 原地修改）。

    合并规则：providers 完全覆盖，permission_mode 非默认值覆盖，
    mcp_servers 按名称合并（同名替换、新名追加），hooks 拼接，
    布尔开关以 True 为生效。
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    按优先级加载多层配置（全局 ~/.owlcode/config.yaml、
    项目 .owlcode/config.yaml、本地 .owlcode/config.local.yaml），
    后加载的配置覆盖前一层。

    Args:
        path: 指定配置文件路径。如果为 None，则按默认多层级策略加载。

    Returns:
        合并后的 AppConfig 对象。

    Raises:
        ConfigError: 未找到任何配置文件或文件解析失败。
    """
    if path is not None:
        if not path.exists():
    ...
    home = Path.home()
    candidates = [
        home / ".owlcode" / "config.yaml",
        cwd / ".owlcode" / "config.yaml",
        cwd / ".owlcode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .owlcode/config.yaml "
            "in project or ~/.owlcode/config.yaml"
        )
    return merged
