...
from owlcode.serialization import (
...
class AuthenticationError(LLMError):
    """认证失败异常（API key 无效或未设置）。"""

    pass


class RateLimitError(LLMError):
    """速率限制异常，包含可选的 retry_after 秒数。"""
...
class LLMClient(ABC):
    ...
    async def stream(
        ...
        """以流式方式调用 LLM，逐块返回 StreamEvent。

        Args:
            conversation: 对话管理器，包含消息历史。
            system: 系统提示词。
            tools: 工具 schema 列表。

        Yields:
            StreamEvent 的子类实例（TextDelta、ToolCallStart 等）。
        """
        yield TextDelta("")
...
class AnthropicClient(LLMClient):
    """Anthropic Messages API 的 LLM 客户端实现。"""
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        Raises:
            AuthenticationError: API key 未找到。
        """
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .owlcode/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    async def stream(
        ...
        对消息和工具列表标记 prompt cache 断点，支持 thinking 模式，
        并将 Anthropic 特定异常转换为内部异常类型。
...
class OpenAIClient(LLMClient):
    """OpenAI Responses API 的 LLM 客户端实现。"""
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .owlcode/config.yaml or via OPENAI_API_KEY env var."
            )
    ...
    async def stream(
        ...
        Args:
            conversation: 对话管理器。
            system: 系统提示词（作为 instructions 传入）。
            tools: 工具 schema 列表。
...
class OpenAICompatClient(LLMClient):
    ...
    async def stream(
        ...
        Args:
            conversation: 对话管理器。
            system: 系统提示词。
            tools: 工具 schema 列表（自动转换为 Chat Completions 格式）。
