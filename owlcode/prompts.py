"""OwlCode 的系统提示词（system prompt）构建。"""
...
class PromptBuilder:
    """系统提示词的构建器，按优先级组装多个 PromptSection。

    分段按优先级升序排列，最终以双换行分隔拼接。
    """
    ...
    def add(self, section: PromptSection) -> PromptBuilder:
        """添加一个提示词分段，返回自身以支持链式调用。

        Args:
            section: 要添加的 PromptSection 对象。
...
IDENTITY_SECTION = PromptSection(
    ...
    priority=0,
    content=(
        "You are OwlCode, an AI programming assistant running in the terminal. "
...
def environment_section(work_dir: str) -> PromptSection:
    ...
    Returns:
        包含工作目录、操作系统和当前日期的 PromptSection。
    """
    lines = [
        "# Environment",
...
def build_plan_mode_reminder(
    ...
    首轮迭代返回完整指令，之后每 _REMINDER_INTERVAL 轮返回一次完整指令，
    其余轮次返回简短提醒。

    Args:
        plan_path: Plan 文件的路径。
        plan_exists: Plan 文件是否已存在。
...
def build_system_prompt(
    ...
    按优先级组装身份、规则、工具使用、语气风格等分段，并附加
    hook 注入的上下文。

    Args:
        hook_prompts: Hook 注入的提示文本列表。
        coordinator_mode: 是否为协调者模式。
        agent_catalog: 可用子 agent 的目录列表。
        custom_instructions: 用户自定义指令。
        skill_section: 技能目录文本。
        memory_section: 记忆文本。
        work_dir: 工作目录路径。
    ...
    """
    if coordinator_mode:
        from owlcode.teams.coordinator import get_coordinator_system_prompt
...
def build_environment_context(
    ...
    """构建环境上下文字符串，作为对话的第一条 user 消息注入。

    Args:
        work_dir: 工作目录路径。
        active_skills: 当前已激活的技能字典。
        skill_catalog: 技能目录文本。
        agent_catalog: Agent 目录文本。
