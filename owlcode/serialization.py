...
from owlcode.conversation import Message
...
def build_anthropic_messages(messages: list[Message]) -> list[dict[str, Any]]:
    """将内部消息列表序列化为 Anthropic Messages API 格式。

    处理 assistant 消息中的 thinking 块、文本和工具调用，
    以及 user 消息中的工具结果。连续的纯文本 user 消息会被合并。

    Args:
        messages: 内部 Message 对象列表。

    Returns:
        符合 Anthropic Messages API 格式的字典列表。
    """
...
def build_openai_input(messages: list[Message]) -> list[dict[str, Any]]:
    """将内部消息列表序列化为 OpenAI Responses API 格式。

    Args:
        messages: 内部 Message 对象列表。

    Returns:
        符合 OpenAI Responses API 输入格式的字典列表。
    """
    result: list[dict[str, Any]] = []
    for m in messages:
        if m.tool_uses:
...
def build_chat_completion_messages(messages: list[Message]) -> list[dict[str, Any]]:
    """将内部消息列表序列化为 OpenAI Chat Completions 格式。
    ...
    Returns:
        符合 OpenAI Chat Completions API 格式的字典列表。
    """
    result: list[dict[str, Any]] = []
    for m in messages:
        if m.tool_uses:
...
def build_messages(messages: list[Message], protocol: str = "anthropic") -> list[dict[str, Any]]:
    ...
    Args:
        messages: 内部 Message 对象列表。
        protocol: 协议类型（"anthropic"、"openai" 或 "openai-compat"）。
