...
class TeamCreateTool(Tool):
    ...
    创建后可配合 Agent 工具的 team_name 参数生成队友，通过 SendMessage 进行通信。
    """
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        Args:
            params: 包含 team_name（团队名称）和 description（可选描述）
                   的 TeamCreateParams。

        Returns:
            ToolResult，创建成功返回团队信息和后端类型。
        """
        p: TeamCreateParams = params  # type: ignore[assignment]

        from owlcode.teams.backend_detect import BackendDetectionError
        ...
        coordinator_note = ""
        from owlcode.teams.coordinator import is_coordinator_mode
        if is_coordinator_mode(self._enable_coordinator_mode):
            from owlcode.agents.tool_filter import apply_coordinator_filter
