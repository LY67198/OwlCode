...
class ReadFile(Tool):
    ...
    支持偏移量和行数限制，结果会被缓存到 FileCache。
    读取的文件会被记录到 FileStateCache 以供后续 EditFile/WriteFile 校验。
    """
    ...
    async def execute(self, params: Params) -> ToolResult:
        ...
        Args:
            params: 包含 file_path（文件路径）、offset（起始行偏移）
                   和 limit（最大行数）的参数。

        Returns:
            ToolResult，output 为带行号的文件内容，文件不存在时 is_error 为 True。
        """
