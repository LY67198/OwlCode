...
class EditFile(Tool):
    ...
    要求 old_string 在文件中恰好出现一次。使用前必须先用 ReadFile 读取文件，
    否则 FileStateCache 会阻止操作。
    """
    ...
    async def execute(self, params: Params) -> ToolResult:
        ...
        Args:
            params: 包含 file_path（文件路径）、old_string（要替换的字符串）
                   和 new_string（替换后的字符串）的参数。

        Returns:
            ToolResult，编辑成功返回确认信息，失败返回错误描述。
        """
