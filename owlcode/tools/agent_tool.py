...
class AgentToolParams(BaseModel):
    ...
    定义子 Agent 的提示词、类型、模型等配置。
    team_name 用于将 Agent 作为队友加入已有团队，否则作为一次性子 Agent 运行。
    """
    prompt: str
    description: str
...
class AgentTool(Tool):
    ...
    支持三种模式：
    1. 指定 subagent_type 加载预定义 Agent 类型
    2. 不指定子类型且 enable_fork 开启时 fork 当前会话
    3. 指定 team_name 作为队友加入已有团队
    支持同步执行和后台运行两种方式。
    """
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        根据参数决定启动模式：队友模式、worktree 隔离模式或普通子 Agent 模式。

        Args:
            params: AgentToolParams，包含 prompt、subagent_type、team_name 等。

        Returns:
            ToolResult，后台运行时返回任务 ID，前台同步运行时返回子 Agent 输出结果。
        """
        ...
        from owlcode.agents.fork import ForkError, build_forked_messages
        from owlcode.agents.parser import AgentDef
        from owlcode.agents.tool_filter import resolve_agent_tools
        from owlcode.agent import Agent as AgentClass
        ...
        if p.subagent_type is None:
            from owlcode.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        创建独立的 worktree、注册队友信息，并根据后端类型（tmux/iTerm2/进程内）
        选择对应的启动方式。

        Args:
            p: AgentToolParams，需包含 team_name。

        Returns:
            ToolResult，启动成功返回队友信息和 worktree 路径。
        """
        ...
        from owlcode.agents.fork import ForkError, build_forked_messages
        from owlcode.agents.parser import AgentDef
        from owlcode.agents.tool_filter import build_teammate_tools
        from owlcode.agent import Agent as AgentClass
        from owlcode.conversation import ConversationManager
        from owlcode.permissions import (
            DangerousCommandDetector,
        ...
        from owlcode.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        """在 tmux 或 iTerm2 pane 中启动队友进程。

        将队友运行在独立的终端 pane 中，使其拥有独立进程空间。

        Args:
            p: AgentToolParams，包含 prompt、subagent_type 等。
            team: 团队实例。
            member: 队友成员信息。
            backend: 后端类型（BackendType.TMUX 或 BackendType.ITERM2）。
            wt: worktree 信息。
            agent_id: agent 唯一标识。
            teammate_name: 队友名称。

        Returns:
            ToolResult，启动成功返回 pane 确认信息。
        """
        ...
        try:
            if backend == BackendType.TMUX:
                from owlcode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from owlcode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    def _select_llm(
        ...
        """根据参数和定义选择合适的 LLM 客户端。

        优先级：params.model > definition.model（非 inherit）> 父 Agent 的 client。

        Args:
            params: AgentToolParams，可能包含 model 覆盖。
            definition: Agent 定义，可能指定默认 model。
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        """在隔离的 git worktree 中执行子 Agent。

        创建全新的 worktree、将工作目录切换到 worktree 路径，
        并在其中运行子 Agent，完成后自动清理。

        Args:
            p: AgentToolParams，包含 prompt 和 subagent_type。

        Returns:
            ToolResult，包含子 Agent 的输出结果。
        """
        if self._worktree_manager is None:
            return ToolResult(
        ...
        from owlcode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        支持常用别名（haiku/sonnet/opus）到完整模型 ID 的映射。

        Args:
            model_alias: 模型别名或完整模型 ID。

        Returns:
            创建的 LLMClient 实例，创建失败返回 None。
        """
        if self._provider_config is None:
            return None

        from owlcode.client import create_client
        from owlcode.config import ProviderConfig
