...
from owlcode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from owlcode.commands.completion import CompletionPopup
from owlcode.commands.handlers import register_all_commands
from owlcode.config import MCPServerConfig, ProviderConfig
from owlcode.hooks import HookContext, HookEngine, load_hooks
from owlcode.conversation import ConversationManager, Message
from owlcode.mcp import MCPManager
from owlcode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from owlcode.permissions import (
...
from owlcode.agents.trace import TraceManager
from owlcode.agents.notification import inject_task_notifications
from owlcode.commands.handlers.tasks import create_tasks_command
...
from owlcode.tools import ToolRegistry, create_default_registry
from owlcode.tools.agent_tool import AgentTool
from owlcode.tools.ask_user import AskUserEvent, AskUserTool
from owlcode.tools.impl.tool_search import ToolSearchTool
from owlcode.tools.load_skill import LoadSkill
from owlcode.worktree.cleanup import start_stale_cleanup_task
from owlcode.worktree.manager import WorktreeManager
from owlcode.commands.handlers.worktree import create_worktree_command
from owlcode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".owlcode", "build", ".gradle"}


def scan_files_for_at(prefix: str, work_dir: str, limit: int = 10) -> list[str]:
    """扫描工作目录中匹配 @ 引用前缀的文件和目录。

    Args:
        prefix: 文件路径前缀（如 "src/m"）。
        work_dir: 工作目录根路径。
        limit: 最大返回条目数。
...
class ChatInput(TextArea):
    """聊天输入框组件，支持历史导航、Tab 补全和命令弹出菜单。

    结合 @文件引用 展开和 /命令 补全，提供与主 agent 交互的输入界面。
    """
    ...
    def load_history(self, work_dir: str) -> None:
        ...
        Args:
            work_dir: 工作目录路径。
        """
        self._history_file = Path(work_dir) / ".owlcode" / "history"
...
class ToolCallBlock(Static, can_focus=True):
    """渲染在聊天区域内的工具调用块，支持折叠/展开。

    显示工具名称、短标题和状态（loading/成功/失败），点击可展开查看详情。
    """
...
class SubAgentBlock(Static, can_focus=True):
    """子 Agent 运行状态块，显示 agent 类型、执行进度和结果。"""
...
class OwlCodeApp(App):
    """OwlCode 的 Textual TUI 应用主类。

    管理 UI 布局、Agent 生命周期、命令分发、权限弹窗、
    MCP 连接、文件历史和会话恢复等核心流程。
    """
    ...
    def __init__(
        ...
        Args:
            providers: LLM provider 配置列表。
            permission_mode: 初始权限模式。
            mcp_servers: MCP 服务器配置列表。
            hook_engine: Hook 引擎实例。
            enable_fork: 是否启用 fork 模式。
            enable_verification_agent: 是否启用验证 agent。
            worktree_config: Worktree 配置。
            teammate_mode: 队友模式。
            enable_coordinator_mode: 是否启用协调者模式。
            driver_class: Textual driver 类。
        """
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        t = RichText()
        t.append(" ┌────┐   ", style="bold color(27)")
        t.append("OwlCode v0.2.0\n", style="color(240)")
        t.append(" │◉ ◉│   ", style="bold color(27)")
        t.append(f"{model}\n" if model else "\n", style="color(240)")
        t.append(" │ ◠ │   ", style="bold color(27)")
        t.append("\n", style="color(240)")
        t.append(" └────┘   ", style="bold color(27)")
        t.append(work_dir, style="color(240)")
        return t
    ...
    def on_mount(self) -> None:
        """应用挂载后的初始化：注册主题，选择 provider。"""
        self.register_theme(_OWLCODE_THEME)
        self.theme = "owlcode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        """选择 provider 并完成完整的初始化流程。

        包括创建 LLM 客户端、权限检查器、agent、技能加载器、
        worktree、子 agent 系统和团队系统。

        Args:
            provider: 要使用的 provider 配置。
        """
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".owlcode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".owlcode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".owlcode" / "permissions.local.yaml",
            ),
        ...
        from owlcode.filehistory import FileHistory
        ...
        from owlcode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from owlcode.config import WorktreeConfig
        ...
        from owlcode.tools.enter_worktree import EnterWorktreeTool
        from owlcode.tools.exit_worktree import ExitWorktreeTool
        ...
        from owlcode.tools.team_create import TeamCreateTool
        from owlcode.tools.team_delete import TeamDeleteTool
        ...
        from owlcode.commands.handlers.trace import create_trace_command
        ...
        from owlcode.tools.synthetic_output import SyntheticOutputTool
    ...
    def set_plan_mode(self, enabled: bool) -> None:
        ...
        Args:
            enabled: True 进入 plan 模式，False 退出并恢复之前模式。
        """
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        """发送用户消息并处理完整的 agent 交互流程。

        包括：等待 MCP 初始化、展开 @引用、预取记忆、渲染 AI 响应流、
        处理工具调用和权限请求、持久化会话记录。

        Args:
            text: 用户输入的文本。
            is_notification: 是否为系统通知触发的消息。
        """
        ...
        if text:
            ...
            from rich.text import Text as RichText
            user_rich = RichText()
            user_rich.append("❯ ", style="bold color(220)")
                ...
                if isinstance(event, ThinkingText):
                    ...
                    if accumulated_text:
                        if streaming_label is not None:
                            await streaming_label.remove()
                        from rich.text import Text as RichText
                        prefix = Static(RichText("●  ", style="bold color(27)"), classes="message")
    ...
    async def _show_plan_approval(self) -> None:
        """在 Plan 模式循环结束后显示内联审批组件。"""
        from owlcode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        """处理 Plan 审批组件的用户应答。"""
        from owlcode.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        """显示内联 AskUser 问题组件并等待用户回答。

        Args:
            event: AskUser 事件，包含问题列表和 future。
        """
        from owlcode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        ...
        Args:
            request: 权限请求对象，包含 future 用于异步等待响应。
        """
        from owlcode.permission_dialog import InlinePermissionWidget
