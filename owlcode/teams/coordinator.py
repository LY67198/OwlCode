...
def is_coordinator_mode(enable_flag: bool = False) -> bool:
    ...
    Args:
        enable_flag: 开关标志，为 False 时始终返回 False。
    ...
    if not enable_flag:
        return False
    val = os.environ.get("OWLCODE_COORDINATOR_MODE", "").lower()
...
def match_session_mode(
    ...
    """将会话模式与环境变量同步，返回状态变更描述。

    Args:
        session_mode: 目标会话模式（"coordinator" 或其他）。
        enable_flag: 是否启用协调器模式。

    Returns:
        如有模式切换则返回描述字符串，否则返回 None。
    """
    if not session_mode:
        return None
    ...
    if session_is_coordinator:
        os.environ["OWLCODE_COORDINATOR_MODE"] = "1"
    else:
        os.environ.pop("OWLCODE_COORDINATOR_MODE", None)
...
def get_coordinator_system_prompt(agent_catalog: list[tuple[str, str]] | None = None) -> str:
    ...
    根据可用的 Agent 类型目录构建完整的协调器行为指令，包括角色定义、
    工具说明、工作流程、验证策略和提示词编写指南。

    Args:
        agent_catalog: (agent_type, description) 元组列表，
            为空时使用默认的 general-purpose 和 Verification。
    ...
    return """You are OwlCode, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
def get_coordinator_user_context(
    ...
    Args:
        worker_tools: 工作进程的可用工具列表，为空时使用默认的 in-process 工具集。

    Returns:
        包含 "workerToolsContext" 键的上下文字典。
    """
    if worker_tools is None:
        from owlcode.agents.tool_filter import IN_PROCESS_TEAMMATE_ALLOWED_TOOLS
